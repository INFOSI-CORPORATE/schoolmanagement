    ...
    <div class="col-md-6 py-2">
        ...
        <select class="form-control" name="fk_courses_id" id="fk_courses_id">
            ...
            @foreach ($courses as $course)
                <option value="{{ $course->id }}" {{ (isset($courseSubject) && $courseSubject->courses->id == $course->id ) ? 'selected' : ((old('fk_courses_id') == $course->id) ? 'selected' : '') }}>{{ $course->name }}</option>
            @endforeach
    ...
    <div class="col-md-6 py-2">
        ...
        <select class="form-control" name="fk_subjects_id" id="fk_subjects_id">
            ...
            @foreach ($subjects as $subject)
                <option value="{{ $subject->id }}" {{ (isset($courseSubject) && $courseSubject->subjects->id == $subject->id ) ? 'selected' : ((old('fk_subjects_id') == $subject->id) ? 'selected' : '') }}>{{ $subject->name }}</option>
            @endforeach
        </select>
    </div>
